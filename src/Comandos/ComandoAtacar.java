...
public class ComandoAtacar implements Comando {
    ...
    public ComandoAtacar(Mapa mapa, String pto_cardinal, String entidad) {
        this.mapa = mapa;
    ...
    public void ejecutar() throws ExcepcionSintaxis, ExcepcionAtacar {
        Personaje p = mapa.getCivilizacion().getPersonajes().get(entidad);
        Edificio E = mapa.getCivilizacion().getEdificios().get(entidad);
        if (p == null && E == null) {
        ...
        if (E == null) {
            if (mapa.checkCoords(p.mover(pto_cardinal))) {
                ...
                throw new ExcepcionAtacar("No se puede atacar a esa casilla");
            }
        } else if (mapa.checkCoords(p.mover(pto_cardinal))) {
            Celda cell = mapa.getCelda(p.mover(pto_cardinal));
            if (cell.isEdificio()) {
                Edificio ef = cell.getEdificio();
                E.atacar(ef);
